...
function ValueDisplay({
  values, editingIndex, onStartEdit, onCommitEdit, onCancelEdit,
  min, max, step, formatValue, label, isRange, isInteracting,
}) {
  ...
  const commitEdit = useCallback((index) => {
    ...
    if (editingIndex === index) {
      return (
        <span className="inline-grid text-[13px]">
          <span className="col-start-1 row-start-1 invisible" style={{ fontVariationSettings: fontWeights.medium }} aria-hidden="true">
          ...
          <span className="col-start-1 row-start-1 flex items-center gap-1">
            ...
            <input
              ...
              className={cn("w-[5ch] bg-transparent text-foreground outline-none border-b border-border text-center", shape.input)}
  ...
  const widestText = isRange ? `${formatValue(max)} — ${formatValue(max)}` : formatValue(max);
  const charCount = (label ? `${label}: ` : "").length + widestText.length;

  return (
    <span
      className="shrink-0 text-[13px] text-muted-foreground text-right transition-[font-variation-settings] duration-100 tabular-nums"
      style={{ fontVariationSettings: isInteracting ? fontWeights.medium : fontWeights.normal, minWidth: `${charCount + 1}ch` }}
    >
      {label && editingIndex === null && <span className="text-muted-foreground">{label}: </span>}
      {isRange ? (
        <React.Fragment>{renderValue(0)}<span className="mx-1 text-muted-foreground/50">—</span>{renderValue(1)}</React.Fragment>
      ) : renderValue(0)}
    </span>
  );
}

function TooltipValue({ value, formatValue, motionX }) {
...
const Slider = forwardRef(
  ({
    ...
    useEffect(() => {
      ...
      const ro = new ResizeObserver(([entry]) => {
        ...
        if (!dragging.current) {
          ...
          hasMounted.current ? animate(motionX0, px0, springs.moderate) : motionX0.set(px0);
          if (isRange && values[1] !== undefined) {
            const px1 = valueToPixel(values[1], min, max, entry.contentRect.width);
            hasMounted.current ? animate(motionX1, px1, springs.moderate) : motionX1.set(px1);
          }
        }
      });
      ro.observe(el);
    ...
    const clampForRange = useCallback((px, thumbIndex) => {
    ...
    const emitChange = useCallback((thumbIndex, newValue) => {
      if (isRange) {
    ...
    const handleRadixChange = useCallback((newValues) => {
      if (dragging.current) return;
      isRange ? onChange(newValues) : onChange(newValues[0]);
    }, [isRange, onChange]);
    ...
    const renderVisualThumb = (index) => {
      ...
      return (
        <motion.span
          ...
          style={{ width: THUMB_SIZE, height: THUMB_SIZE, marginTop: -THUMB_SIZE / 2, x: motionX, position: "absolute", top: "50%", left: 0, zIndex: 10 }}
          initial={false} transition={springs.moderate}
        >
          <motion.span
            ...
            transition={springs.fast}
            style={{ backgroundColor: "white", boxShadow: "0 1px 4px rgba(0,0,0,0.15), 0 0 0 1px rgba(0,0,0,0.06)" }}
          />
          <motion.span
            className="absolute rounded-full border border-[#6B97FF] pointer-events-none"
            initial={{ opacity: 0, width: THUMB_SIZE + 4, height: THUMB_SIZE + 4 }}
    ...
    return (
      ...
      >
        <div
          ...
          onPointerLeave={() => { setIsHovered(false); setHoverThumbIndex(null); }}
        >
          <SliderPrimitive.Root
            value={values} onValueChange={handleRadixChange}
            min={min} max={max} step={step} disabled={disabled}
            className="absolute inset-0 opacity-0 pointer-events-none" style={{ height: THUMB_SIZE }}
          >
            <SliderPrimitive.Track className="w-full h-full"><SliderPrimitive.Range /></SliderPrimitive.Track>
            <SliderPrimitive.Thumb
              className="block outline-none" style={{ width: THUMB_SIZE, height: THUMB_SIZE }}
          ...
          >
            <div className="absolute cursor-pointer" style={{ left: -8, right: -8, top: 0, bottom: 0 }}
              onPointerDown={handlePointerDown} onPointerMove={handlePointerMove} onPointerUp={handlePointerUp} />
            
            <div
              className={cn("absolute left-0 right-0 bg-black/10 backdrop-blur-md", shape.bg)}
              style={{
                height: isHovered || isPressed ? 8 : TRACK_HEIGHT,
                top: isHovered || isPressed ? 8 + (THUMB_SIZE - 8) / 2 : 8 + (THUMB_SIZE - TRACK_HEIGHT) / 2,
